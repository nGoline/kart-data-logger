...
/* One scene: the same figures the firmware would be holding at that moment. */
typedef struct {
    const char *name;
    int      lap, speed;
    uint32_t clock_ms, best_ms, last_ms, ref_ms, run_ms;
...
static void play(const char *dir, const scene_t *sc) {
    ...
    if (sc->last_ms) {
        snprintf(b, sizeof b, "%u:%02u.%03u", sc->last_ms / 60000,
                 (sc->last_ms % 60000) / 1000, sc->last_ms % 1000);
        char bb[24];
        snprintf(bb, sizeof bb, "%u:%02u.%03u", sc->best_ms / 60000,
                 (sc->best_ms % 60000) / 1000, sc->best_ms % 1000);
    ...
    if (sc->has_ref) {
        uint32_t pred = (uint32_t)((int32_t)sc->ref_ms + (int32_t)(sc->delta_s * 1000.0f));
        snprintf(b, sizeof b, "%u:%02u.%03u", pred / 60000,
                 (pred % 60000) / 1000, pred % 1000);
        ui_dash2_set_predicted(b);
    }

    snprintf(b, sizeof b, "%u:%02u.%03u", sc->clock_ms / 60000,
             (sc->clock_ms % 60000) / 1000, sc->clock_ms % 1000);
    ui_dash2_set_lap_clock(b);

    for (int k = 0; k < 3; k++)
        ui_dash2_set_sector((dash2_sector_t)k, sc->ss[k], sc->sd[k]);
    if (sc->run_ms) {
        snprintf(b, sizeof b, "%u.%01u", sc->run_ms / 1000, (sc->run_ms % 1000) / 100);
...
int main(int argc, char **argv) {
    ...
    static const scene_t scenes[] = {
        ...
        { "3_down", 4, 41, 48905, 62628, 62807, 62628, 8400, 1.24f, 0.62f,
        ...
        { "4_recover", 6, 58, 71400, 62628, 78225, 62628, 6100, 15.62f, -0.24f,
          DASH2_DELTA_SLOWER, true,
        ...
        { "5_best", 5, 58, 320, 62140, 62140, 62140, 0, -0.488f, -0.488f,
        ...
        { "7_lap1", 1, 57, 34120, 0, 0, 0, 9400, 0.0f, 0.0f, DASH2_DELTA_NONE, false,
        ...
        { "6_widest", 18, 112, 528888, 82444, 88888, 82444, 88800, 88.88f, 88.88f,
          DASH2_DELTA_SLOWER, true,
        ...
        { "9_stopped", 0, 0, 0, 0, 0, 0, 0, 0.0f, 0.0f, DASH2_DELTA_NONE, false,
          { DASH2_SECTOR_PENDING, DASH2_SECTOR_PENDING, DASH2_SECTOR_PENDING }, { 0, 0, 0 } },
    };
