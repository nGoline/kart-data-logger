 ...
 *
 * Drives an imaginary kart round an oval and emits TelemetryMsg frames at the
 * same 25 Hz the real receiver does, so demo mode can run the whole live
 * pipeline (LapManager, the sector gates, the reference trace, the delta, the
 * lap clock) rather than posing a static screen. Nothing here is a mock of the
 * UI: the numbers go in the front of the real code and come out on the real
 * dashboard.
 *
 * It carries its own finish line and split gates, because the track selected on
 * the device may be miles away or (as track 1 currently is) have no split gates
 * at all, and a demo that shows an empty sector rail demonstrates nothing.
 *
 * Deliberately free of Arduino and LVGL: tools/lapreplay/demo verifies the
 * geometry on the host, which is the only practical way to know the gates are
 * wound the right way round. A gate with its posts swapped is not a crash, it
 * is a lap counter that silently never increments.
 * ========================================================================= */
...
public:
    ...
    void begin(uint64_t epochBaseMs) {
    ...
    /* Advance the simulation to `nowMs` (a millis() reading) and emit one frame
     * per STEP_MS of elapsed time. Call it in a while loop the way the real GPS
     * drain does, so a slow pass catches up instead of dropping frames. */
    bool update(uint32_t nowMs, TelemetryMsg &out) {
        ...
        /* After a stall (an SD flush, a screen change) run forward at most a
         * second. Replaying the whole gap would teleport the kart through a
         * gate, and a jumped gate is a missed lap. */
    ...
    /* Gate posts, ordered left then right as LapManager wants them: it rejects
     * a crossing whose travel direction is not 90 degrees counter-clockwise
     * from the left-to-right post vector. See gateGeometry(). */
    ...
    uint32_t lapsDriven() const { return _lap; }

private:
    /* Oval, in metres from the centre. 630 m round, which at the pace below is
     * a lap in the high thirties — long enough to read as a lap, short enough
     * that a demo shows several without anyone waiting. */
    ...
    /* Near Kartodromo Granja Viana, so a demo session's coordinates look like
     * somewhere rather than like the Gulf of Guinea. */
    ...
    /* Cheap deterministic noise, +/- roughly half a metre, so the delta reads
     * the way it will on track. A perfectly smooth demo would advertise a
     * steadiness the GPS cannot deliver. */
    double jitter() {
    ...
    double speedKmh() const {
        ...
        /* A driver warming up: pace climbs towards a plateau, with enough
         * per-lap variation to throw in off laps. This shape is chosen, not
         * arbitrary. A flat-plus-noise pace makes the first timed lap the
         * fastest by luck and every later one slower, so the panel goes red
         * and stays red and the demo never shows green or purple at all.
         * Rising-with-variation gives new bests at laps 1, 2, 5, 6, 10, 11 of
         * twelve, with slower laps between them. */
        double warm  = W0 + (1.0 - W0) * (1.0 - exp(-(double)_lap / W_TAU));
        double pace  = warm * (1.0 + 0.025 * sin(_lap * 1.30)
                                   + 0.015 * sin(_lap * 0.55 + 0.4));
        /* And a shape that moves round the lap as well as between laps:
         * without it every lap would differ from the reference by a constant
         * and the live delta would be a flat line, which is precisely the
         * behaviour the live delta exists to replace. */
