...
/* Bottom band: the lap clock keeps the left, and the width the old LAST
 * readout was spending on a duplicate of the header's lap time goes to the
 * sector cells, which were previously 12 px in the corner. */
#define BAND_Y       238
#define BAND_H        78
...
typedef struct {
    ...
    /* The delta bar when it runs off its scale, so "a second down" and "fifteen
     * seconds down" are not drawn identically. */
    uint32_t peg;
} dash2_tokens_t;
...
static const dash2_tokens_t TOK_NIGHT = {
    ...
    .best = 0xB44DFF, .best_deep = 0x2A0F3A, .peg = 0xFFB020,
};
static const dash2_tokens_t TOK_DAY = {
    ...
    .best = 0xC77DFF, .best_deep = 0x2A0F3A, .peg = 0xFFC940,
};
...
void ui_dash2_init(lv_obj_t *parent)
{
     ...
     * label's own width never changes and KM/H stays put — aligning the unit to
     * a content-sized label made it jump between "9", "63" and "112".
     *
     * Right- rather than left-aligned so the ones digit is the one that is
     * pinned. That is the digit that changes every sample; the hundreds slot
     * simply fills in on the odd occasion a kart sees three figures. It also
     * keeps the number tight against its unit instead of stranding it a whole
     * digit-cell away whenever the speed has two digits. */
    ...
    /* Predicted lap: where this lap is heading, which is the number actually
     * worth spending a glance on. */
    ...
    /* Said in words, because the alternative is placeholder digits: the Barlow
     * faces are digits-only by design, so a blanked "--.--" at this size is a
     * row of thick bars and a fat dot. It reads as debris rather than absence,
     * and blanking the delta and the prediction at once doubles the mess. */
...
static void refresh_delta(void)
{
    ...
    /* Always the same caption, including through the session-best flash. The number
     * under this caption is the PREDICTED lap either way, so labelling it
     * SESSION BEST during the purple hold mislabels it — the lap that was the
     * session best is the one in the header. Purple already says "best" on its
     * own; that is what the colour is for in motorsport. */
    lv_label_set_text(lbl_pred_cap, "OPTIMAL");
...
/* Split-relative, not cumulative, and that distinction is the whole reason this
 * element is worth having. A cumulative bar pegs the instant you have a real
 * off and then sits at the end of its scale for the remaining fifty seconds,
 * saying nothing about the corners you are actually driving. Measured on the
 * 12-lap fixture: the split delta stays inside 0.73 s on a normal lap and only
 * 4.4% of all samples run off scale. */
static void refresh_bar(void)
{
    if (!bar_fill) return;
    ...
    uint32_t c = pegged ? T.peg : (s_split_delta < 0.0f ? T.good : T.bad);
    ...
    for (int i = 0; i < DASH2_SECTOR_COUNT; i++) {
        ...
        lv_obj_set_style_border_opa(chip[i],
            (st == DASH2_SECTOR_FASTER || st == DASH2_SECTOR_SLOWER ||
             st == DASH2_SECTOR_TIMED) ? LV_OPA_40 : LV_OPA_COVER, 0);
        lv_obj_set_style_text_color(chip_tag[i], C(col), 0);
        lv_obj_set_style_text_color(chip_val[i], C(col), 0);

        /* A closed sector shows its delta big. The one being driven shows its
         * running split, which is where that readout lives now that the
         * bottom-left panel is the lap clock. One not yet reached shows a small
         * quiet dash: "--.--" in the big face is chunky bars that pull more
         * attention than the real numbers beside them, which is backwards. */
        if (st == DASH2_SECTOR_FASTER || st == DASH2_SECTOR_SLOWER) {
            float d = s_sectors[i].delta;
            lv_obj_set_style_text_font(chip_val[i], DASH2_FONT_PILL, 0);
            /* One decimal, not two. Tabular digits take the widest digit's width,
             * which makes "±N.NN" about 100 px against an 86 px cell. One decimal
             * fits at 77 px and is the more honest figure anyway: with hAcc around
             * a metre this is good to roughly a tenth, so the second decimal had a
             * whole column to itself for noise.
             *
             * Past ten seconds the fraction goes too — the exact figure tells you
             * nothing you do not already know from the lap being ruined. */
            const char *sign = d < 0 ? "-" : "+";
            float a = fabsf(d);
            ...
            lv_obj_set_style_text_font(chip_val[i], &lv_font_montserrat_16, 0);
            lv_label_set_text(chip_val[i], "--");
        }
    }
}
...
/* Back, with a proper home: the sector cell being driven. */
